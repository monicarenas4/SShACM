...
retaken_image = folder_name + '/Image_228_blur_2_scaled_107.jpg'

img_ref = cv2.resize(cv2.imread(reference_image),
                     (1600, 1200),
                     interpolation=cv2.INTER_AREA)

y_ref, x_ref, c_ref = img_ref.shape

img_ret = cv2.resize(cv2.imread(retaken_image),
                     (x_ref, y_ref),
                     interpolation=cv2.INTER_AREA)
...
HEIGHT = int(img_ref.shape[0] / NUM_ROWS)
WIDTH = int(img_ref.shape[1] / NUM_COLS)
...
def scores_function(retake, reference, cp=5, threshold=3, corner=2):
    ...
    imAligned, h = align_images(retake, reference)
    ...
    for cpu in cpus:
        ...
        for row in range(NUM_ROWS):
            ...
            for col in range(NUM_COLS):
                ...
                pool.apply_async(scores_function,
                                 args=(ret, ref),
